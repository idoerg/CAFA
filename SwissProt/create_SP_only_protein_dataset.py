...
def insert_into_db(record, taxon, GAFFIELDS):
    ...
    try:
        ...
        if len(GAFFIELDS) == 17:
            c.execute("create table SP_gaf(db varchar(20), db_id varchar(20), db_symbol varchar(20), qualifier varchar(20), GO_Term varchar(40), db_ref varchar(40), evidence varchar(3), With varchar(20), ontology char(1), db_name varchar(100), synonym varchar(200), db_type varchar(40), taxid varchar(20), Date date, source varchar(40), ann_ext varchar(20), gene_product varchar(20))")
            c.executemany("insert into SP_gaf values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", record)
            conn.commit()
        elif len(GAFFIELDS) == 15:
            c.execute("create table SP_gaf(db varchar(20), db_id varchar(20), db_symbol varchar(20), qualifier varchar(20), GO_Term varchar(40), db_ref varchar(40), evidence varchar(3), With varchar(20), ontology char(1), db_name varchar(100), synonym varchar(200), db_type varchar(40), taxid varchar(20), Date date, source varchar(40))")
            c.executemany("insert into SP_gaf values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", record)
            conn.commit()
...
def extract_gaf(rec, outfile, GAFFIELDS, record, sp_id, taxon):
    ...
    if sp_id.has_key(rec['DB_Object_ID']):    
        ...
        if len(GAFFIELDS) == 15:
            t = (rec['DB'], rec['DB_Object_ID'], rec['DB_Object_Symbol'], ('|'.join(rec['Qualifier'])), rec['GO_ID'], ('|'.join(rec['DB:Reference'])), rec['Evidence'], ('|'.join(rec['With'])), rec['Aspect'], rec['DB_Object_Name'], ('|'.join(rec['Synonym'])), rec['DB_Object_Type'], ('|'.join(rec['Taxon_ID'])), rec['Date'], rec['Assigned_By'])
        elif len(GAFFIELDS) == 17:
            t = (rec['DB'], rec['DB_Object_ID'], rec['DB_Object_Symbol'], ('|'.join(rec['Qualifier'])), rec['GO_ID'], ('|'.join(rec['DB:Reference'])), rec['Evidence'], ('|'.join(rec['With'])), rec['Aspect'], ('|'.join(rec['DB_Object_Name'])), ('|'.join(rec['Synonym'])), rec['DB_Object_Type'], ('|'.join(rec['Taxon_ID'])), rec['Date'], rec['Assigned_By'], rec['Annotation_Extension'], rec['Gene_Product_Form_ID'])
