...
import collections
import re, shlex, sys
from ..Data import OntologyTerm, OntologyGraph
...
class OboIterator(OntoIterator):
    ...
    def _strip_comment(self, line):
        pos = line.find('!')
        if pos > 0:
            # check to make sure it isn't escaped
            try:
                for token in shlex.split(line):
                    if '!' in token:
                        tokenstart = line.find(token) - 1
                        tokenend = tokenstart + len(token) + 1
                        if tokenstart > 0 and tokenend < len(line) and line[tokenstart] == '"' \
                                and line[tokenend] == '"':

                            return line
            except ValueError:
                pass # this means the comment should be stripped
            return line[0:pos]
        else:
            return line
