...
'''                                                                                                                                          
    This script takes a uniprot-goa gpi format file as input and converts it into a table in a sqlite database.                              
    A single table, by the name of uniprot-goa, will be created that will hold all values from the gpi file                                  
...
def gpi_parser(rec, outfile, record):
    ...
    if rec.has_key('Gene_Product_Properties'):
        t = (rec['DB_Object_ID'], rec['DB_Object_Symbol'], ('|'.join(rec['DB_Object_Name'])), ('|'.join(rec['DB_Object_Synonym'])), rec['DB_Object_Type'], rec['Taxon'], rec['Parent_Object_ID'], ('|'.join(rec['DB_Xref'])), ('|'.join(rec['Gene_Product_Properties'])))
    else:
        t = (rec['DB'], rec['DB_subset'], rec['DB_Object_ID'], rec['DB_Object_Symbol'], rec['DB_Object_Name'], ('|'.join(rec['DB_Object_Synonym'])), rec['DB_Object_Type'], rec['Taxon'], ('|'.join(rec['Annotation_Target_Set'])))
...
if __name__ == '__main__':
    ...
    for rec in parser:
        if rec.has_key('Gene_Product_Properties'):
            ...
            c.execute("create table uniprot_goa(db varchar(20), db_subset varchar(40), db_id varchar(20), db_symbol varchar(100), db_name varchar(200), db_synonym varchar(100), db_type varchar(40), taxid varchar(40), annotation_target_set varchar(40))")
        break
